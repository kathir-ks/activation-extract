...
class ActivationExtractionConfig:
    ...
    activation_type: str = 'residual'  # 'mlp', 'attn', or 'residual' - where to extract activations from
    ...
    def __post_init__(self):
        ...
        if self.activation_type not in ['mlp', 'attn', 'residual']:
            raise ValueError(f"activation_type must be 'mlp', 'attn', or 'residual', got: {self.activation_type}")
...
def load_model_and_tokenizer(model_path: str, config: QwenConfig, layers_to_extract: List[int],
    ...
    jax_model = create_model_with_hooks(config, layers_to_extract=layers_to_extract, activation_type=activation_type)
...
def main():
    ...
    parser.add_argument('--activation_type', type=str, choices=['mlp', 'attn', 'residual'],
                        help="Type of activation to extract: 'mlp' (MLP output before residual), "
                             "'attn' (attention output before residual), or 'residual' (final output after both residuals)")
    ...
    model, tokenizer, params = load_model_and_tokenizer(
        cfg.model_path, config, cfg.layers_to_extract, cfg.activation_type
    )
