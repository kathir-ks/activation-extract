...
def convert_hf_weights(hf_model, config: ModelConfig) -> Dict:
    ...
    if hasattr(hf_model.model, 'embed_tokens'):
        params['embed_tokens'] = {
            'embedding': jnp.array(hf_model.model.embed_tokens.weight.cpu().numpy())
        }
    ...
    for i in range(config.num_hidden_layers):
        ...
        layer_params['self_attn'] = {
            'q_proj': {
                'kernel': jnp.array(layer.self_attn.q_proj.weight.T.cpu().numpy()),
                'bias': jnp.array(layer.self_attn.q_proj.bias.cpu().numpy())
            },
            'k_proj': {
                'kernel': jnp.array(layer.self_attn.k_proj.weight.T.cpu().numpy()),
                'bias': jnp.array(layer.self_attn.k_proj.bias.cpu().numpy())
            },
            'v_proj': {
                'kernel': jnp.array(layer.self_attn.v_proj.weight.T.cpu().numpy()),
                'bias': jnp.array(layer.self_attn.v_proj.bias.cpu().numpy())
            },
            'o_proj': {
                'kernel': jnp.array(layer.self_attn.o_proj.weight.T.cpu().numpy())
            }
        }
        
        # MLP
        layer_params['mlp'] = {
            'gate_proj': {'kernel': jnp.array(layer.mlp.gate_proj.weight.T.cpu().numpy())},
            'up_proj': {'kernel': jnp.array(layer.mlp.up_proj.weight.T.cpu().numpy())},
            'down_proj': {'kernel': jnp.array(layer.mlp.down_proj.weight.T.cpu().numpy())}
        }
        
        # Norms
        layer_params['input_layernorm'] = {
            'scale': jnp.array(layer.input_layernorm.weight.cpu().numpy())
        }
        layer_params['post_attention_layernorm'] = {
            'scale': jnp.array(layer.post_attention_layernorm.weight.cpu().numpy())
        }
    ...
    # Final norm
    params['norm'] = {
        'scale': jnp.array(hf_model.model.norm.weight.cpu().numpy())
    }
    
    # LM head
    if not config.tie_word_embeddings:
        params['lm_head'] = {
            'kernel': jnp.array(hf_model.lm_head.weight.T.cpu().numpy())
        }
