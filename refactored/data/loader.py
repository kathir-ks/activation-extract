...
Provides utilities for loading ARC datasets from various formats:
- JSONL files with optional machine-based sharding
- Sharded datasets with automatic shard claiming
- Prompt creation with data augmentation
"""
...
def load_arc_dataset_from_shard(
    ...
    """
    from .sharding import ShardManager, load_shard_chunks
    
    if verbose:
        print(f"\n{'='*70}")
        print("Loading from Sharded Dataset")
        print(f"{'='*70}")
    ...
    if shard_info is None:
        raise RuntimeError(
            ...
            "All shards may be in use or completed."
        )
...
def create_prompts_from_dataset(
    ...
    try:
        from ..arc.augmentation import (
            apply_data_augmentation,
            get_random_color_map,
            set_random_seed
        )
        from ..arc.prompting import create_prompts_from_task
    except ImportError:
