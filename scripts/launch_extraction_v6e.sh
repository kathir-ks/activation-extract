#!/bin/bash
# =============================================================================
# Resilient extraction launcher for v6e-64 TPU pod
#
# Run with nohup:
#   nohup bash scripts/launch_extraction_v6e.sh > launch_v6e.log 2>&1 &
#
# This script:
# 1. Sets up all TPU workers (code + deps)
# 2. Launches multihost extraction on v6e-64
...
# Extraction config
LAYER=12
GCS_PREFIX="activations/layer12_v6e64_gridchunk_50k"
CHECKPOINT_PREFIX="checkpoints/v6e64_gridchunk_layer12"
...
create_tarball() {
    ...
    tar czf "$TARBALL" \
        ...
        --exclude='extraction.log' --exclude='launch.log' --exclude='launch_v6e.log' \
        activation-extract/
    cd "$REPO_DIR"
...
log "=========================================="
log "Resilient Extraction Launcher (v6e-64)"
log "=========================================="
