#!/bin/bash
# =============================================================================
# SAE Training Launcher v4 — JumpReLU architecture (8x expansion)
#
# Purpose: direct A/B against v2 8x TopK — same dict size, same data, same
# training budget. Only architecture and sparsity mechanism differ.
#
# JumpReLU uses a learnable per-feature threshold; sparsity is controlled via
# --l1_coeff (L0 penalty with STE). TopK enforces k-active; JumpReLU does not.
#
# Run with nohup:
#   nohup bash scripts/launch_sae_training_v4_jumprelu.sh > sae_training_v4_jumprelu.log 2>&1 &
# =============================================================================
...
HIDDEN_DIM=896
DICT_SIZE=7168        # 8x expansion (matches v2 for direct arch comparison)
K=32                  # Unused by jumprelu but harmless to pass
L1_COEFF=3e-3         # L0 penalty coefficient (tune if L0 lands wrong)
...
launch_training() {
    ...
    gcloud compute tpus tpu-vm ssh "$TPU_NAME" --zone="$ZONE" --worker=all --command="
        ...
        nohup python3 -u -m sae.scripts.train \
            ...
            --checkpoint_gcs_prefix sae_checkpoints/layer19_jumprelu_896d_v4 \
...
log "=========================================="
log "SAE Training Launcher v4 JumpReLU (v5e-64)"
log "=========================================="
log "TPU: $TPU_NAME ($ZONE)"
log "Architecture: $ARCHITECTURE (hidden=$HIDDEN_DIM, dict=$DICT_SIZE, l1_coeff=$L1_COEFF)"
...
log "Checkpoint prefix: sae_checkpoints/layer19_jumprelu_896d_v4"
log "Dtype: $DTYPE"
log "=========================================="
...
while true; do
    ...
    if ! check_training_running; then
        ...
        if [ "$completed" = "DONE" ]; then
            log "=========================================="
            log "SAE TRAINING V4 JUMPRELU COMPLETED SUCCESSFULLY!"
