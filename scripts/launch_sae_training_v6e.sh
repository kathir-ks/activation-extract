...
#
# Trains a Sparse Autoencoder on pre-extracted layer 12 activations
...
# Data source (same-region GCS bucket)
GCS_PATH="gs://arc-data-europe-west4/activations/layer12_v1_50k_v5litepod-64"
LAYER_INDEX=12
...
setup_worker() {
    ...
    gcloud compute tpus tpu-vm ssh "$TPU_NAME" --zone="$ZONE" --worker="$w" --command="
        ...
        pip install -q flax optax 'transformers>=4.38.0' 'jinja2>=3.1.0' torch tqdm gcsfs 'google-cloud-storage>=2.14.0' numpy 2>/dev/null &&
        echo 'Worker $w ready'
    " 2>/dev/null | tail -1
...
launch_training() {
    ...
    gcloud compute tpus tpu-vm ssh "$TPU_NAME" --zone="$ZONE" --worker=all --command="
        ...
        nohup python3 -u -m sae.scripts.train \
            ...
            --checkpoint_gcs_prefix sae_checkpoints/layer12_topk_v6e64 \
